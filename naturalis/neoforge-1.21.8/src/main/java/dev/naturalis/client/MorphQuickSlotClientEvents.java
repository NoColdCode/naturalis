...
public final class MorphQuickSlotClientEvents {
    ...
    private static void sendRemorphedMorph(ResourceLocation morphId) {
        EntityType<?> type = BuiltInRegistries.ENTITY_TYPE.get(morphId).map(holder -> holder.value()).orElse(null);
        if (type == null) {
            return;
        }
    ...
    public static void onRenderGui(RenderGuiEvent.Post event) {
        MorphQuickSlotOverlay.render(event.getGuiGraphics(), event.getPartialTick().getGameTimeDeltaPartialTick(false));
    }
}
