 ...
 * Scent trails: several thin strands that weave horizontally and stay near the ground.
...
public final class ScentTrailWorldRenderer {
    ...
    public static void onRenderLevel(RenderLevelStageEvent.AfterParticles event) {
        ...
        renderWorldPass(event.getPoseStack(), event.getCamera(), event.getPartialTick().getGameTimeDeltaPartialTick(false));
    ...
    public static void onRenderFramePost(RenderFrameEvent.Post event) {
        ...
        Matrix4f matrix = new Matrix4f(mc.gameRenderer.getProjectionMatrix(partial)).mul(poseStack.last().pose());



        MultiBufferSource.BufferSource buffers = mc.renderBuffers().bufferSource();

        renderToBuffer(buffers, matrix, camera, partial, 1.0F);
    ...
    private static void renderWorldPass(PoseStack poseStack, Camera camera, float partialTick) {
