...
public final class WolfVisionClientEvents {
    ...
    private static final ResourceLocation INVERT_SHADER =
        new ResourceLocation("minecraft", "invert");
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (!RuleFlagsClientCache.isColorFilterEnabled()) {
            disableShader(mc);
        ...
        boolean scentOverlay = dev.naturalis.client.perception.MorphSniffClientState.isScentVisionActive()
            && morphId != null
            && InstinctManager.hasSmellSense(morphId);
        if (scentOverlay && desiredShader != null) {
            desiredShader = MorphSniffVisionEffects.resolveSniffChain(desiredShader);
        }
    ...
    private static void startShader(Minecraft mc, ResourceLocation shader) {
        ...
        try {
            renderer.getClass().getMethod("setPostEffect", ResourceLocation.class).invoke(renderer, shader);
            return;
        } catch (ReflectiveOperationException ignored) {
            // Fall through.
        }

        // 1.20.1 path: loadEffect expects the fully qualified post-chain json location.
        ResourceLocation legacyPath = new ResourceLocation(
            shader.getNamespace(),
        ...
        try {
            renderer.getClass().getMethod("loadEffect", ResourceLocation.class).invoke(renderer, shader);
        } catch (ReflectiveOperationException ignored) {
            // No-op fallback.
        }
    }
