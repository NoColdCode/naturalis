...
public class MorphArmorForgeMenu extends AbstractContainerMenu {
    ...
    private void addForgeSlots(MorphArmorForgeBlockEntity be) {
        var handler = be.getItemHandler();

        addSlot(new SlotItemHandler(handler, MorphArmorForgeBlockEntity.SLOT_ORB, 35, 35) {
            @Override
        ...
        addSlot(new SlotItemHandler(handler, MorphArmorForgeBlockEntity.SLOT_MATERIAL, 80, 35) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return handler.isItemValid(MorphArmorForgeBlockEntity.SLOT_MATERIAL, stack);
            }
        });

        addSlot(new SlotItemHandler(handler, MorphArmorForgeBlockEntity.SLOT_OUTPUT, 125, 35) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return false;
            }
