...
public final class MorphQuickSlotManager {
    ...
    /** Ensures bridge handlers are registered before packet consumers are wired. */
    ...
    public static void handleSelection(ServerPlayer player, int slotIndex, @Nullable ResourceLocation clientMorphId) {
        ...
        MorphQuickSlotDebug.event(
            "server",
            "handleSelection slot=" + slotIndex + " clientMorph=" + clientMorphId + " target=" + target
        );

        if (target != null) {
            if (!isMorphAvailable(player, target)) {
                player.displayClientMessage(
                    net.minecraft.network.chat.Component.translatable("message.naturalis.quick_slot.morph_failed"),
                    true
                );
                MorphQuickSlotDebug.event("server", "handleSelect rejected unavailable morph " + target);
                return;
            }
            if (!MorphQuickSlotMorphUtil.applyMorph(player, target)) {
                player.displayClientMessage(
                    net.minecraft.network.chat.Component.translatable("message.naturalis.quick_slot.morph_failed"),
                    true
                );
        ...
        MorphQuickSlotDebug.event("server", "handleSelection no target for slot=" + slotIndex);
    }
    ...
    private static List<ResourceLocation> listAvailableMorphs(ServerPlayer player) {
        ...
        for (String key : getXpMorphKeys(player)) {
            ResourceLocation parsed = parseMorphId(key);
