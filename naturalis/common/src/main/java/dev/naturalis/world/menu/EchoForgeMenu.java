...
public class EchoForgeMenu extends AbstractContainerMenu {
    ...
    public EchoForgeMenu(int containerId, Inventory playerInventory, FriendlyByteBuf buffer) {
        this(containerId, playerInventory, (EchoForgeBlockEntity) playerInventory.player.level().getBlockEntity(buffer.readBlockPos()));
    }

    public EchoForgeMenu(int containerId, Inventory playerInventory, BlockPos pos) {
        this(containerId, playerInventory, (EchoForgeBlockEntity) playerInventory.player.level().getBlockEntity(pos));
    }
    ...
    private void addForgeSlots(Container inventory) {
        ...
        addSlot(new Slot(inventory, 1, 104, 35) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.is(filled);
            }
        });
        addSlot(new Slot(inventory, 2, 92, 57) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.is(filled);
            }
        });
        addSlot(new Slot(inventory, 3, 68, 57) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.is(filled);
            }
        });
        addSlot(new Slot(inventory, 4, 56, 35) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.is(filled);
            }
        ...
        addSlot(new Slot(inventory, 5, 80, 35) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return false;
            }
