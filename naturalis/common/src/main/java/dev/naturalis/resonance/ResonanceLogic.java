...
public final class ResonanceLogic {
    ...
    private static final net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> SPEED_EFFECT =
        CompatAccess.resolveMobEffect("MOVEMENT_SPEED", "SPEED");
    private static final net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> NAUSEA_EFFECT =
        CompatAccess.resolveMobEffect("CONFUSION", "NAUSEA");
    private static final net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> STRENGTH_EFFECT =
        CompatAccess.resolveMobEffect("DAMAGE_BOOST", "STRENGTH");
    private static final net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> RESISTANCE_EFFECT =
        CompatAccess.resolveMobEffect("DAMAGE_RESISTANCE", "RESISTANCE");
    private static final net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> SLOWNESS_EFFECT =
        CompatAccess.resolveMobEffect("MOVEMENT_SLOWDOWN", "SLOWNESS");
    private static final net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> JUMP_BOOST_EFFECT =
    ...
    private static final ResourceLocation ADV_FIRST_BOND = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/first_bond");
    private static final ResourceLocation ADV_RESONANCE_ONLINE = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/resonance_online");
    private static final ResourceLocation ADV_MILESTONE_ONE = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/milestone_one");
    private static final ResourceLocation ADV_MILESTONE_TWO = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/milestone_two");
    private static final ResourceLocation ADV_MILESTONE_THREE = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/milestone_three");
    private static final ResourceLocation ADV_HUMANITY_DRIFTING = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/humanity_drifting");
    private static final ResourceLocation ADV_HUMANITY_SPLIT = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/humanity_split");
    private static final ResourceLocation ADV_HUMANITY_FERAL = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/humanity_feral");
    private static final ResourceLocation ADV_HUMANITY_PRIMAL = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/humanity_primal");
    private static final ResourceLocation ADV_HUMANITY_LOST = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/humanity_lost");
    private static final ResourceLocation ADV_REBIRTH_ACCEPTED = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/rebirth_accepted");
    private static final ResourceLocation ADV_TOKEN_MINOR = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/token_minor");
    private static final ResourceLocation ADV_TOKEN_GREATER = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/token_greater");
    private static final ResourceLocation ADV_TOOL_DECAY = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/tool_decay");
    private static final ResourceLocation ADV_TOOL_REJECTED = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/tool_rejected");
    private static final ResourceLocation ADV_MORPH_SLEEPLESS = ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "resonance/morph_sleepless");
    ...
    private static final TagKey<Block> CONSECRATED_BEDS = TagKey.create(
        ...
        ResourceLocation.fromNamespaceAndPath(NaturalisMod.ID, "consecrated_beds")
    );
    ...
    public static void tick(ServerPlayer player) {
        ...
        if (player.tickCount % 40 == 0 && !player.hasEffect(CompatAccess.naturalisMobEffectHolder("morph_binding"))) {
            ...
            if (player.getRandom().nextDouble() < chance) {
                player.addEffect(new MobEffectInstance(CompatAccess.naturalisMobEffectHolder("morph_binding"), 8 * 20, 0, true, false, true));
            }
        }
    }

    /** @return adjusted damage amount. */
    public static float modifyOutgoingDamage(ServerPlayer player, LivingEntity victim, float damage) {
        if (!ResonanceManager.isResonanceEnabled(player)) {
            return damage;
        }
        ...
        if (bonded == null || current == null || !bonded.equals(current)) {
            if (current == null) {
                return applyHumanToolDamagePenalty(player, damage);
            }
            return damage;
        }
        ...
        if (ResonanceManager.getHumanityStage(player).ordinal() >= ResonanceManager.HumanityStage.PRIMAL.ordinal()) {
            ...
            tag.putBoolean(LAST_RITE_FULL_HEALTH_HIT, fullHealthHit || damage >= victimHealthBefore);
        }
        ...
        if (ResonanceManager.getHumanityStage(player).ordinal() >= ResonanceManager.HumanityStage.FERAL.ordinal()) {
            if (victim instanceof Player defendingPlayer && defendingPlayer.isBlocking()) {
    ...
    public static void onLivingDeath(LivingEntity victim, net.minecraft.world.damagesource.DamageSource source) {
        ...
        if (!(source.getEntity() instanceof ServerPlayer player) || victim == player) {
            return;
        }
    ...
    public static void onFoodFinished(ServerPlayer player, ItemStack consumed) {
        if (!ResonanceManager.isResonanceEnabled(player)) {
            return;
        }
        if (consumed.get(net.minecraft.core.component.DataComponents.FOOD) == null) {
            return;
        }
    ...
    /** @return result to cancel with, or null to pass. */
    public static InteractionResult tryRightClickItem(ServerPlayer player, ItemStack stack) {
        ...
        if (stack.is(CompatAccess.naturalisItem("humanity_token_5")) || stack.is(CompatAccess.naturalisItem("humanity_token_10"))) {
            ...
            int amount = stack.is(CompatAccess.naturalisItem("humanity_token_10")) ? 10 : 5;
        ...
        if (stack.is(CompatAccess.naturalisItem("memory_token"))) {
        ...
        if (!stack.is(CompatAccess.naturalisItem("morph_orb"))) {
            return null;
        }
        ...
        ItemStack orbStack = new ItemStack(CompatAccess.naturalisItem("morph_orb"));
        ...
        // Cooldown: do not cancel — allow normal morph-orb use.
        return null;
    }

    public static InteractionResult tryEntityInteract(ServerPlayer helper, net.minecraft.world.entity.Entity targetEntity, ItemStack stack) {
        if (!(targetEntity instanceof ServerPlayer target)) {
            return null;
        }
        if (!stack.is(CompatAccess.naturalisItem("rehumanizer"))) {
            return null;
        }
        ...
        ForceHumanBridge.forceHuman(target);
        stack.shrink(1);
    ...
    public static float modifyIncomingDamage(ServerPlayer player, float damage) {
        if (!ResonanceManager.isResonanceEnabled(player)) {
            return damage;
        }
    ...
    public static InteractionResult tryRightClickBlock(ServerPlayer player, BlockPos pos, ItemStack stack) {
        ...
        if (shouldToolFumble(player, stack)) {
            player.displayClientMessage(Component.translatable("message.naturalis.resonance.tool_fumble"), true);
            return InteractionResult.FAIL;
        }
        return null;
    ...
    public static boolean shouldCancelLeftClickBlock(ServerPlayer player, ItemStack stack) {
        if (shouldToolFumble(player, stack)) {
            player.displayClientMessage(Component.translatable("message.naturalis.resonance.tool_fumble"), true);
            return true;
        }
    ...
    public static boolean shouldCancelAttack(ServerPlayer player) {
    ...
    /** @return adjusted break speed, or null if unchanged. */
    public static Float modifyBreakSpeed(ServerPlayer player, float originalSpeed) {
        if (!ResonanceManager.isResonanceEnabled(player)) {
            return null;
        }
        if (CurrentMorphUtil.getCurrentMorphId(player) != null) {
        ...
        if (multiplier < 1.0F) {
            return Math.max(0.1F, originalSpeed * multiplier);
        }
        return null;
    }
    ...
    private static void applyTerritorialMap(ServerPlayer player, CompoundTag tag, long now) {
        ...
        if (player.level() instanceof ServerLevel level) {
            ...
            level.playSound(null, nearest.getX(), nearest.getY(), nearest.getZ(), SoundEvents.NOTE_BLOCK_BASEDRUM, SoundSource.HOSTILE, volume, pitch);
        }
    }
    ...
    private static float applyHumanToolDamagePenalty(ServerPlayer player, float damage) {
        ItemStack held = player.getMainHandItem();
        if (!isToolLike(held)) {
            return damage;
        }
    ...
    private static boolean stashPlayerInventoryIntoBody(ServerPlayer player, LivingEntity body, ServerLevel level) {
        ...
        NonNullList<ItemStack> stash = NonNullList.withSize(slots, ItemStack.EMPTY);
        ...
        if (!carried.isEmpty()) {
            hadAnyItems = true;
            stash.set(inventorySlots, carried.copy());
        }

        try {
            CompoundTag lootTag = CompatAccess.serializeItemStacks(stash, level.registryAccess());
            if (hadAnyItems && lootTag.isEmpty()) {
                // Serialization failed silently on this runtime; keep inventory instead of losing it.
                return true;
            }

            CompatAccess.getPersistentData(body).put(REBIRTH_LOOT_TAG, lootTag);
            CompatAccess.getPersistentData(body).putInt(REBIRTH_LOOT_SLOTS_TAG, slots);
            CompatAccess.getPersistentData(body).putString(REBIRTH_OWNER_TAG, player.getStringUUID());

            ResourceLocation currentMorph = CurrentMorphUtil.getCurrentMorphId(player);
            if (currentMorph != null) {
                CompatAccess.getPersistentData(body).putString(REBIRTH_MORPH_TAG, currentMorph.toString());
            }
    ...
    private static void restorePlayerInventoryFromStash(ServerPlayer player, NonNullList<ItemStack> stash) {
        int limit = Math.min(player.getInventory().getContainerSize(), stash.size());
    ...
    private static void releaseRebirthLoot(LivingEntity victim, ServerLevel level) {
        CompoundTag data = CompatAccess.getPersistentData(victim);
        ...
        NonNullList<ItemStack> stash = NonNullList.withSize(slots, ItemStack.EMPTY);
        CompatAccess.loadItemStacks(stash, level.registryAccess(), lootTag);
    ...
    private static void grantAdvancement(ServerPlayer player, ResourceLocation id) {
        ...
        AdvancementHolder root = server.getAdvancements().get(ADV_ROOT);
        if (root != null) {
