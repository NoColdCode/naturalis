...
public final class MorphAcquisition {
    ...
    public static boolean acquire(ServerPlayer player, ResourceLocation mobId, CompoundTag shapeData) {
        try {
            if (!dev.naturalis.survivalas.SurvivalAsRuntime.isAcquireAllowed(mobId)) {
                player.displayClientMessage(dev.naturalis.survivalas.SurvivalAsMessages.cannotChange(), true);
                return false;
            }
            ...
            // Directly set the 2nd shape Ã¢â‚¬â€ bypasses UNLOCK_SHAPE event so Remorphed kill gates can't block this.
