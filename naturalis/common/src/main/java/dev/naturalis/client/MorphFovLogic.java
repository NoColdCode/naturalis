...
public final class MorphFovLogic {
    ...
    private MorphFovLogic() {
    }

    public static double getActiveMorphFovMultiplier(Minecraft mc) {
    ...
    public static double getMorphFovMultiplier(ResourceLocation morphId) {
        ...
        if (AVIAN_MORPHS.contains(path)
            || matchesAny(path, "bird", "crow", "raven", "eagle", "hawk", "falcon", "owl", "vulture", "gull", "duck", "goose", "swan", "toucan", "emu", "shoebill", "potoo")) {
            return AVIAN_FOV_MULTIPLIER;
        }
        if (HORSE_MORPHS.contains(path)) {
            return HORSE_FOV_MULTIPLIER;
        }
        if (CANINE_MORPHS.contains(path) || matchesAny(path, "wolf", "fox", "hyena", "dingo", "jackal")) {
            return CANINE_FOV_MULTIPLIER;
        }
        if (FELINE_MORPHS.contains(path) || matchesAny(path, "tiger", "lion", "leopard", "jaguar", "cheetah")) {
            return FELINE_FOV_MULTIPLIER;
        }
        if (SPIDER_MORPHS.contains(path)) {
            return SPIDER_FOV_MULTIPLIER;
        }
        if (AQUATIC_MORPHS.contains(path) || matchesAny(path, "orca", "whale", "squid", "octopus", "lobster", "crab", "seal", "shark", "hammerhead")) {
            return AQUATIC_FOV_MULTIPLIER;
        }
        if (PREY_MORPHS.contains(path) || matchesAny(path, "capybara", "rhino", "elephant", "hippo", "manatee", "bison", "buffalo", "deer", "moose")) {
            return PREY_FOV_MULTIPLIER;
        }
        return 1.0D;
    }

    public static Double getVanillaFovMultiplier(String path) {
        return switch (path) {
